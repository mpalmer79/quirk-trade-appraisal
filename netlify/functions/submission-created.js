...
export async function handler(event) {
  ...
  // Honeypot: ignore bots (optional: add your own field name if you use one)
  if (data.botField || data.honeypot) {
    return { statusCode: 200, body: "ok" };
  }

  // Extract uploaded files (Netlify stores in payload)
  let fileUrls = [];
  try {
    const files = payload.files || [];
    fileUrls = files.map(f => f.url).filter(Boolean);
  } catch {}
  ...
  const prettyHtml = `
    ...
    <table cellpadding="6" cellspacing="0" border="0" style="border-collapse:collapse;">
      ${rows.map(([k,v]) => `
        <tr>
          <th align="left" style="text-transform:capitalize;font-family:system-ui,Segoe UI,Roboto,Helvetica,Arial;font-size:14px;color:#111827;padding:6px 10px 6px 0;">
            ${htmlEscape(k)}
          </th>
          <td style="font-family:system-ui,Segoe UI,Roboto,Helvetica,Arial;font-size:14px;color:#111827;padding:6px 0;">
            ${htmlEscape(v)}
          </td>
        </tr>
      `).join("")}
  ...
  // Build safe attachments from file URLs (size-limited)
  ...
  // Optional: backup to Sheets webhook
  try {
